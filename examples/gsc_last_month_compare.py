...
def _get_historical_weeks(weeks_back:int = 1) -> tuple:
    ...
    4) end date of the period (same number of days) previous
    '''
    last_week_end = dt.date.today() - relativedelta(days = (weeks_back * dt.date.today().weekday()) + 2 )
...
class GSCReport():
    ...
    def __init__(self,
        ...
        if date_range.lower() == 'm':
            ...
            self.prev_period_end = _get_historical_months(1)
            self.period_name = 'Monthly'
        elif date_range.lower() == 'w':
            self.last_period_start,\
            self.last_period_end,\
            self.prev_period_start,\
            self.prev_period_end = _get_historical_weeks(1)
            self.period_name = 'Weekly'
        else:
            raise TypeError('Please select a correct date range. Either "M" for monthly or "W" for weekly')
    ...
    def _url_subsections(self,subsection:str) -> None:
        ...
        sub = self.compare_df.loc[self.compare_df.index.get_level_values(0).str.contains(subsection)].copy()
        ...
        ci = sub.loc[sub.index.get_level_values(column_index).isin(['Clicks','Impressions'])].copy()
        p = sub.loc[sub.index.get_level_values(column_index).isin(['Position','CTR'])].copy()
    ...
    def compare(self) -> None:
        ...
        self.compare_obj = Compare(self._clean(self.previous_period),self._clean(self.last_period))
    ...
    def highs_and_lows(self,percent_sort:bool = False,filter_zero_values:bool = True) -> None:
        '''
        ...
        for x in ['Clicks','Impressions','CTR','Position']:
            column_index = len(self.compare_df.index.names)-1
            temp = self.compare_df.loc[self.compare_df.index.get_level_values(column_index)==x].sort_values('change',ascending=False)
            ...
            if self.print_out:
                print(f"\n--{x}--\n")
                if x != 'Position':
                    print(f"Biggest Lossers: {temp.tail(self.results_rows)[::-1]}\n\nBiggest Winners: {temp.head(self.results_rows)}")
                else:
                    print(f"Biggest Lossers: {temp.head(self.results_rows)}\n\nBiggest Winners: {temp.tail(self.results_rows)[::-1]}")
    ...
    def check_previous_top_sites(self,num_sites:int = 10,filter_zero_values:bool = True) -> None:
        '''
        ...
        for x in ['Clicks','Impressions','CTR','Position']:
            column_index = len(self.compare_df.index.names)-1
            old_top = self.compare_df.loc[self.compare_df.index.get_level_values(column_index)==x].sort_values('from',ascending=False)
            ...
            if x != 'Position':
                ...
                self._send(f"{x}'s Previous Top Positions", old_top.tail(num_sites)[::-1])
    ...
    def from_list(self,sites_data:list,parent_folder = None):
        ...
        parent_folder: ID of parent folder (your new periodly folder will be stored in)
            type: str
        """
        folder_id = self.out_sheet.auth.drive.create_folder(f"{self.dim_proper} GSC {self.period_name} Reports - {self.last_period_start}", parent_folder)
        ...
        for site in sites_data:
            ...
            self.out_sheet.create_sheet(f"{site} {self.dim_proper} Report - {self.last_period_start}",folder_id=folder_id)
        ...
        # update summary
        self.out_sheet.create_sheet(f"{self.period_name} {self.dim_proper} Summary Report - {self.last_period_start}",folder_id=folder_id)
